...
class UserAuthentificationMessageViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        myButton.titleLabel?.font = UIFont().Body3_R14
        myButton.status = .fill
        myButton.addTarget(self, action: #selector(authButtonClicked), for: .touchUpInside)
        myButton.setTitle("인증하고 시작하기", for: .normal)
    ...
    @objc func authButtonClicked() {
        ...
        Auth.auth().signIn(with: credential) { success, error in
            if error == nil {
                ...
                let alert = UIAlertController(title: "실패", message: "인증코드가 틀렸습니다!", preferredStyle: .alert)
                let ok = UIAlertAction(title: "확인", style: .default)
